...
class ndk_SIGNUPController extends Controller
{
    ...
    public function ndksignupSubmit(Request $request)
    {
        ...
        if ($lastCustomer) {
            $newCustomerID = 'KH' . str_pad((substr($lastCustomer->ndkMaKhachHang, 2) + 1), 3, '0', STR_PAD_LEFT);  // e.g., KH001, KH002, etc.
        } else {
        ...
        $ndkkhachhang->ndkMatKhau = $request->ndkMatKhau; // Encrypt the password
